"""
Transport-agnostic base class for all lab device families.

``DeviceBase`` is the single copy of everything device classes share
regardless of how they talk to the hardware (pyserial, TCP, vendor DLL):

- the uniform constructor contract:
  ``device_id, port, baudrate, logger, sink, test_mode,
...
Transports plug in via three hooks: ``_open_transport()``,
``_close_transport()`` and ``_transport_desc()``. The pyserial
implementation lives in ``serial_device.py::SerialDeviceBase``; the CGC
DLL implementation is the P6 lab layer (``CGCDevice``).
...
class DeviceBase:
    """
    Transport-agnostic base class for lab devices with housekeeping,
    logging, telemetry and explicit test mode.

    Subclasses implement the transport hooks (``_open_transport()``,
    ``_close_transport()``, ``_transport_desc()``) and ``hk_monitor()``
    (one housekeeping cycle: read every channel and report it via
    ``log_sample()``). Reads should branch on ``self.test_mode`` and
    return simulated values (helpers: ``_sim_uniform()``,
    ``_sim_choice()``) instead of touching hardware.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            port: Transport endpoint (e.g. "COM4" on Windows, "/dev/ttyUSB0"
                on Linux); appears in the aligned log columns.
            ...
            test_mode: If True, the device never opens the transport and
            ...
            thread_lock: Optional external lock for device communication.
        ...
        # Setup thread lock (for device communication)
    ...
    def _open_transport(self) -> None:
        """Open the physical transport (subclasses implement)."""
        raise NotImplementedError(
            f"{type(self).__name__} must implement _open_transport()"
        )

    def _close_transport(self) -> None:
        """Close the physical transport (subclasses implement)."""
        raise NotImplementedError(
            f"{type(self).__name__} must implement _close_transport()"
        )
