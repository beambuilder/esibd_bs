...
class HiScroll12(PfeifferBaseDevice):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # err_hist = error history [P:360-362], newest first, blank = empty.
        self._sim_state = {"enabled": True, "standby": False,
    ...
    def acknowledge_error(self) -> None:
        """Acknowledge errors (ErrorAckn [P:009]) — required before a pump
        that tripped into an error state accepts a new start."""
        if self.test_mode:
            self.log_event("info", "error acknowledged (simulated)")
            return
    ...
    def get_error_history(self, slot: int) -> str:
        ...
        response = self.query_parameter(359 + slot)
