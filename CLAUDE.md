...
- **Telemetry**: `src/devices/telemetry.py` — `TelemetrySink` protocol + stdlib `SQLiteSink` (schema per workspace ADR-0002). Devices accept `sink=`; `log_sample()` writes numeric values to it (`sim` flag set from `test_mode`). `SQLiteSink.write_command()` (2026-07-10) fills the `commands` audit table (ctrl-API operations, read by the lab_master alert engine); deliberately NOT part of the protocol — wrapper sinks (Explorer's ChannelThrottle) must keep satisfying it unchanged.
